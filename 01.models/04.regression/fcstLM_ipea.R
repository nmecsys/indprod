...
m1 <- pimFactors(database$dados3, lagsy = NULL, lagsf = NULL, n.fatores = 1, p = 1, log = F,  diff = T,
                 carnaval = carnaval, pim_cajuste = pim_cajuste)
#saveRDS(m1$fcst_x,"06.shiny/data/fcstx.rds")
m <- pimLM(database$dados3, lagsy = c(1,11,12), outDFM = m1$fcst_x, carnaval = carnaval, log = F, diff = T)

# exercício fora da amostra -----------------------------------------------------
m_out <-  pimLMFcst(database$dados3, lagsy = c(1,11,12), outDFM = m1$fcst_x, log = F, diff = T, carnaval = carnaval, pim_cajuste = pim_cajuste, h = 48)
...
saveRDS(resultados, "01.models/04.regression/regression_model_ipea.rds")

resultadosLM_ipea <- resultados
